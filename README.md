...
把扒谱 `style_hint` 这类逗号串**拆成一行一个标签**，在前端面板里逐行换「类别 + 取值」（下拉 / 常用点选 / 手输），再拼回逗号串。适合"只改速度感觉、保留调性与结构"这类微调 —— 不用重写整条 style。

| 参数（界面名） | 类型 | 说明 |
|------|------|------|
| 风格 | STRING | 接扒谱【风格底稿】（也可直接粘贴）。面板会拆行；没连上游时手输的文本就是输入 |
| 分隔符 | STRING | 输出拼回用的分隔符，默认 `, `；输入始终按 逗号/分号/顿号/换行 拆。支持字面量 `\n` |
| 替换状态 | STRING | **面板维护的 JSON，不用手改**。为空 / 损坏 / 与本次输入不一致时本节点原样输出输入 |

| 输出 | 类型 | 说明 |
|------|------|------|
| 风格 | STRING | 替换后的逗号串 → 接生成节点的 `style` |

**面板用法**：每行 = 「类别下拉 + 取值下拉 + 手输框 + 常用点选 chips + ↺复原/↑↓/✕删除」；底部实时预览输出，顶部「+ 添加标签」「重新解析」。类别共 17 种：调性 / 速度BPM / 速度感觉 / 拍号 / 和声走向 / 曲式 / 段落结构 + 词表识别的 9 类（语种 / 流派 / 人声类型 / 音色质地 / 唱法技巧 / 乐器 / 情绪氛围 / 节奏编曲 / 年代制作）+ 其他。

解析规则（针对扒谱真实产出）：逗号既分隔标签、也可能在标签内部 —— `harmony built on C, Am, F, G`（和弦名继续吸收）与 `sections: intro → verse×2 → …` 会并回一行；`92 BPM` 与 `slow` **故意拆两行**（好单独改）。行值**照抄原文**，只有你主动改才变。

**需先执行一次**：扒谱节点不返回 ui，面板拿不到执行前的上游值 → 第一次 Queue 输出=原样透传、面板自动填充；改完再 Queue 一次即生效（第二次上游有缓存，很快）。

接法：`扒谱.【风格底稿】─► 风格标签替换.【风格】`，`风格标签替换.【风格】─► 生成.【风格】`（可直接替换直连，也可接在 风格标签/合并节点 之后做二次微调）。
